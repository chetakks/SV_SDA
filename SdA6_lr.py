...
class SdA6(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, n_ins=None,
        ...
        #self.predictions = self.logLayer.y_pred(self.y)
...
#         
#         # Shared variable
#         #l_r = theano.shared(numpy.array(0.1, dtype=theano.config.floatX)) 
#             
#         updates = []
#         for param, gparam in zip(self.params, gparams):
#             updates.append((param, param - gparam * l_r))
#             
#         train_fn = theano.function([index,l_r], 
...
#                                     (index + 1) * batch_size]})
    ...
    def build_test(self, datasets, batch_size, learning_rate):
        '''Generates a function `train` that implements one step of
        finetuning, a function `validate` that computes the error on
        a batch from the validation set, and a function `test` that
        computes the error on a batch from the testing set
        ...
        (test_set_x2, test_set_y2) = datasets[2]
        (test_set_x3, test_set_y3) = datasets[2]
        (test_set_x4, test_set_y4) = datasets[2]

        # compute number of minibatches for training, validation and testing
        n_test_batches0 = test_set_x0.get_value(borrow=True).shape[0]
        n_test_batches0 /= batch_size
        n_test_batches1 = test_set_x1.get_value(borrow=True).shape[0]
        n_test_batches1 /= batch_size
        n_test_batches2 = test_set_x2.get_value(borrow=True).shape[0]
        n_test_batches2 /= batch_size
        n_test_batches3 = test_set_x3.get_value(borrow=True).shape[0]
        n_test_batches3 /= batch_size
        n_test_batches4 = test_set_x4.get_value(borrow=True).shape[0]
        n_test_batches4 /= batch_size


        index = T.lscalar('index')  # index to a [mini]batch

        
        test_score_i0 = theano.function([index], self.errors,
                 givens={
                   self.x: test_set_x0[index * batch_size:
                                      (index + 1) * batch_size],
                   self.y: test_set_y0[index * batch_size:
                                      (index + 1) * batch_size]})
        test_score_i1 = theano.function([index], self.errors,
                 givens={
                   self.x: test_set_x1[index * batch_size:
                                      (index + 1) * batch_size],
                   self.y: test_set_y1[index * batch_size:
                                      (index + 1) * batch_size]})
        test_score_i2 = theano.function([index], self.errors,
                 givens={
                   self.x: test_set_x2[index * batch_size:
                                      (index + 1) * batch_size],
                   self.y: test_set_y2[index * batch_size:
                                      (index + 1) * batch_size]})
        test_score_i3 = theano.function([index], self.errors,
                 givens={
                   self.x: test_set_x3[index * batch_size:
                                      (index + 1) * batch_size],
                   self.y: test_set_y3[index * batch_size:
                                      (index + 1) * batch_size]})
        test_score_i4 = theano.function([index], self.errors,
                 givens={
                   self.x: test_set_x4[index * batch_size:
                                      (index + 1) * batch_size],
                   self.y: test_set_y4[index * batch_size:
                                      (index + 1) * batch_size]})


        # Create a function that scans the entire test set
        def test_score0():
            return [test_score_i0(i) for i in xrange(n_test_batches0)]
        def test_score1():
            return [test_score_i1(i) for i in xrange(n_test_batches1)]
        def test_score2():
            return [test_score_i2(i) for i in xrange(n_test_batches2)]
        def test_score3():
            return [test_score_i3(i) for i in xrange(n_test_batches3)]
        def test_score4():
            return [test_score_i4(i) for i in xrange(n_test_batches4)]
        
        return test_score0, test_score1, test_score2, test_score3, test_score4
